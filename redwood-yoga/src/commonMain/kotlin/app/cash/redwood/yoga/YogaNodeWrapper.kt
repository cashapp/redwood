...
class YogaNodeWrapper private constructor(private var mNativePointer: YGNode) : YogaNode() {
  ...
  private var mChildren: MutableList<YogaNodeWrapper>? = null
  ...
  override fun calculateLayout(width: Float, height: Float) {
    val nativePointers: Array<YGNode?>?
    val nodes: Array<YogaNodeWrapper>?
    freeze(null)
    val n = mutableListOf<YogaNodeWrapper>()
    n.add(this)
    for (i in n.indices) {
      val parent = n[i]
