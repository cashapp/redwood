...
internal class DiffProducingWidgetChildren(
  ...
  override fun insert(index: Int, widget: Widget<Nothing>) {
    ...
    protocolState.putWidget(widget)
    protocolState.append(ChildrenDiff.Insert(id, tag, widget.id, widget.type, index))
  }

  override fun remove(index: Int, count: Int) {
    for (i in index until index + count) {
      protocolState.removeWidget(ids[i])
    }
    ids.remove(index, count)
    protocolState.append(ChildrenDiff.Remove(id, tag, index, count))
  }

  override fun move(fromIndex: Int, toIndex: Int, count: Int) {
    ids.move(fromIndex, toIndex, count)
    protocolState.append(ChildrenDiff.Move(id, tag, fromIndex, toIndex, count))
  }
